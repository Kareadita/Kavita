...
export class BulkSelectionService {
        ...
        if (isForwardSelection) {
          this.debugLog(
            'Selecting ' +
              ...
              this.prevIndex +
              ' to ' +
              this.dataSourceMax[this.prevDataSource]
          );
          this.selectCards(
            this.prevDataSource,
            this.prevIndex,
          ...
          this.debugLog(
            'Selecting ' + dataSource + ' cards from ' + 0 + ' to ' + index
          );
  ...
  isCardSelected(dataSource: DataSource, index: number) {
    if (
      this.selectedCards.hasOwnProperty(dataSource) &&
      this.selectedCards[dataSource].hasOwnProperty(index)
    ) {
  ...
  hasSelections() {
    ...
    return (
      keys.filter((key) => {
        return (
          Object.values(this.selectedCards[key]).filter((item) => item).length >
          0
        );
      }).length > 0
    );
  ...
  totalSelections() {
    ...
    const keys = Object.keys(this.selectedCards);
    keys.forEach((key) => {
      sum += Object.values(this.selectedCards[key]).filter(
        (item) => item
      ).length;
    });
    return sum;
  ...
  getActions(callback: (action: Action, data: any) => void) {
    ...
    if (
      Object.keys(this.selectedCards).filter((item) => item === 'series')
        .length > 0
    ) {
      return this.actionFactory
        .getSeriesActions(callback)
        .filter((item) => allowedActions.includes(item.action));
    }

    if (
      Object.keys(this.selectedCards).filter((item) => item === 'bookmark')
        .length > 0
    ) {
