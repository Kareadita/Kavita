...
export class ActionFactory2Service {
  ...
  getSeriesActions(shouldRenderFunc: ActionShouldRenderFunc<Series> = this.basicReadRender) {
    const actions = this.applyCallbackToList(
      this.seriesActions,
      this.dummyCallback,  // old callback stays as-is
      shouldRenderFunc
    );

    // Layer on callback2 for series
    this.applyCallbackToList2(actions,
      (action, entity) => this.actionService.handleSeriesAction2(action, entity)
    );
  ...
  private applyCallback2(action: ActionItem<any>, callback: ActionResultCallback<any>, shouldRenderFunc: ActionShouldRenderFunc<any>) {
  ...
  private applyCallbackToList2<T>(list: Array<ActionItem<T>>,
    ...
    actions.forEach((action) => this.applyCallback2(action, callback, shouldRenderFunc));
