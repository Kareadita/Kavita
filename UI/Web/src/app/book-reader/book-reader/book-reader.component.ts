...
import {
  KEY_CODES,
  UtilityService,
} from 'src/app/shared/_services/utility.service';
...
export class BookReaderComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('readingHtml', { static: false })
  readingHtml!: ElementRef<HTMLDivElement>;
  @ViewChild('readingSection', { static: false })
  readingSectionElemRef!: ElementRef<HTMLDivElement>;
  @ViewChild('stickyTop', { static: false })
  stickyTopElemRef!: ElementRef<HTMLDivElement>;
  ...
  isNextPageDisabled() {
    ...
    const condition =
      (this.nextPageDisabled ||
        this.nextChapterId === CHAPTER_ID_DOESNT_EXIST) &&
      this.pageNum + 1 > this.maxPages - 1;
  ...
  isPrevPageDisabled() {
    ...
    const condition =
      (this.prevPageDisabled ||
        this.prevChapterId === CHAPTER_ID_DOESNT_EXIST) &&
      this.pageNum === 0;
    if (this.layoutMode !== BookPageLayoutMode.Default) {
  ...
  get IsNextChapter(): boolean {
    ...
    return (
      this.pageNum + 1 >= this.maxPages &&
      currentVirtualPage === totalVirtualPages
    );
  }
  /**
   * Determines if we show << or <
  ...
  get PageHeightForPagination() {
    if (this.layoutMode === BookPageLayoutMode.Default) {
      return (
        (this.readingSectionElemRef?.nativeElement?.scrollHeight || 0) -
        this.topOffset * 2 +
        'px'
      );
    }
  ...
  handleScrollEvent() {
    ...
    if (Object.keys(this.pageAnchors).length !== 0) {
      ...
      const verticalOffset =
        this.scrollService.scrollPosition + this.document.body.offsetHeight / 2;

      const alreadyReached = Object.values(this.pageAnchors).filter(
        (i: number) => i <= verticalOffset
      );
      if (alreadyReached.length > 0) {
        this.currentPageAnchor = Object.keys(this.pageAnchors)[
          alreadyReached.length - 1
        ];
      } else {
    ...
    this.memberService
      ...
      .subscribe((hasProgress) => {
        if (!hasProgress) {
          this.toggleDrawer();
          this.toastr.info(
            'You can modify book settings, save those settings for all books, and view table of contents from the drawer.'
          );
        }
      });
    ...
    if (event.key === KEY_CODES.RIGHT_ARROW) {
      this.movePage(
        this.readingDirection === ReadingDirection.LeftToRight
          ? PAGING_DIRECTION.FORWARD
          : PAGING_DIRECTION.BACKWARDS
      );
  ...
  loadPrevChapter() {
    ...
    if (
      this.prevChapterId === CHAPTER_ID_NOT_FETCHED ||
      (this.prevChapterId === this.chapterId && !this.prevChapterPrefetched)
    ) {
      this.readerService
        .getPrevChapter(
          this.seriesId,
  ...
  addLinkClickHandlers() {
    ...
    links.forEach((link: any) => {
      link.addEventListener('click', (e: any) => {
        if (!e.target.attributes.hasOwnProperty('kavita-page')) {
          return;
        }
        var page = parseInt(e.target.attributes['kavita-page'].value, 10);
        if (this.adhocPageHistory.peek()?.page !== this.pageNum) {
          this.adhocPageHistory.push({
            page: this.pageNum,
            scrollOffset: window.pageYOffset,
          });
        }

        var partValue = e.target.attributes.hasOwnProperty('kavita-part')
          ? e.target.attributes['kavita-part'].value
          : undefined;
  ...
  promptForPage() {
    const question =
      'There are ' +
      (this.maxPages - 1) +
      ' pages. What page do you want to go to?';
  ...
  loadPage(part?: string | undefined, scrollTop?: number | undefined) {
    ...
    this.bookService
      .getBookPage(this.chapterId, this.pageNum)
      .pipe(take(1))
      .subscribe((content) => {
        ...
        setTimeout(() => {
          ...
          const imgs =
            this.readingSectionElemRef.nativeElement.querySelectorAll('img');
    ...
    if (part !== undefined && part !== '') {
      ...
      if (this.layoutMode === BookPageLayoutMode.Default) {
        ...
        if (this.pagingDirection === PAGING_DIRECTION.BACKWARDS) {
          setTimeout(() =>
            this.scrollService.scrollToX(
              this.readingHtml.nativeElement.scrollWidth,
              this.readingHtml.nativeElement
            )
          );
        } else {
          setTimeout(() =>
  ...
  setPageNum(pageNum: number) {
    ...
    if (this.pageNum >= this.maxPages - 10) {
      ...
      if (
        !this.nextChapterPrefetched &&
        this.nextChapterId !== CHAPTER_ID_DOESNT_EXIST
      ) {
        //   && !this.nextChapterDisabled
        this.readerService
  ...
  prevPage() {
    ...
    if (this.layoutMode !== BookPageLayoutMode.Default) {
      ...
      if (currentVirtualPage > 1) {
        ...
        this.scrollService.scrollToX(
          (currentVirtualPage - 2) * pageWidth,
          this.readingHtml.nativeElement
        );
        this.handleScrollEvent();
        return;
  ...
  nextPage(event?: any) {
    ...
    // We need to handle virtual paging before we increment the actual page
    if (this.layoutMode !== BookPageLayoutMode.Default) {
      const [currentVirtualPage, totalVirtualPages, pageWidth] =
        this.getVirtualPage();

      if (currentVirtualPage < totalVirtualPages) {
        // +0 apparently goes forward 1 virtual page...
        this.scrollService.scrollToX(
          currentVirtualPage * pageWidth,
          this.readingHtml.nativeElement
        );
        this.handleScrollEvent();
        return;
  ...
  getPageWidth() {
    ...
    const margin =
      this.readingSectionElemRef.nativeElement.clientWidth *
      (parseInt(this.pageStyles['margin-left'], 10) / 100) *
      2;
    const columnGap = 20;
    return (
      this.readingSectionElemRef.nativeElement.clientWidth - margin + columnGap
    );
  }
  ...
  getVirtualPage() {
    if (
      this.readingHtml === undefined ||
      this.readingSectionElemRef === undefined
    )
      return [1, 1, 0];
    ...
    if (totalScroll - delta === 0) {
      ...
      currentVirtualPage = Math.min(
        Math.max(1, Math.round((scrollOffset + pageWidth) / pageWidth)),
        totalVirtualPages
      );
    }
  ...
  getFirstVisibleElementXPath() {
    ...
    const intersectingEntries = Array.from(
      this.readingHtml.nativeElement.querySelectorAll(
        'div,o,p,ul,li,a,img,h1,h2,h3,h4,h5,h6,span'
      )
    )
  ...
  updateReaderStyles(pageStyles: PageStyle) {
    ...
    const resumeElement: string | null | undefined =
      this.getFirstVisibleElementXPath();
    ...
    const individualElementStyles = Object.entries(this.pageStyles).filter(
      (item) => elementLevelStyles.includes(item[0])
    );
  ...
  updateColorTheme(theme: BookTheme) {
    // Remove all themes
    Array.from(this.document.querySelectorAll('style[id^="brtheme-"]')).forEach(
      (elem) => elem.remove()
    );
    ...
    this.renderer.appendChild(
      this.document.querySelector('.reading-section'),
      styleElem
    );
  ...
  updateWidthAndHeightCalcs() {
    this.windowHeight = Math.max(
      this.readingSectionElemRef.nativeElement.clientHeight,
      window.innerHeight
    );
    ...
    this.scrollbarNeeded =
      this.readingHtml.nativeElement.clientHeight >
      this.reader.nativeElement.clientHeight;
  }
  ...
  scrollTo(partSelector: string) {
    ...
    if (this.layoutMode === BookPageLayoutMode.Default) {
      ...
      setTimeout(
        () =>
          this.scrollService.scrollTo(fromTopOffset, this.reader.nativeElement),
        10
      );
    } else {
      setTimeout(() =>
        (element as Element).scrollIntoView({ block: 'start', inline: 'start' })
      );
    }
  }
  ...
  getXPathTo(element: any): string {
    ...
    for (let sibling of siblings) {
      if (sibling === element) {
        return (
          this.getXPathTo(element.parentNode) +
          '/' +
          element.tagName +
          '[' +
          (ix + 1) +
  ...
  toggleFullscreen() {
    ...
    if (this.isFullscreen) {
      ...
      this.readerService.enterFullscreen(this.reader.nativeElement, () => {
        ...
        this.renderer.setStyle(
          this.reader.nativeElement,
          'background',
          this.themeService.getCssVariable('--bs-body-color')
        );
        if (!this.darkMode) {
          this.renderer.setStyle(
            this.reader.nativeElement,
  ...
  setupPageAnchors() {
    this.pageAnchors = {};
    this.currentPageAnchor = '';
    const ids = this.chapters
      .map((item) => item.children)
      .flat()
      .filter((item) => item.page === this.pageNum)
      .map((item) => item.part)
  ...
  showClickToPaginateVisualOverlay() {
    ...
    if (
      this.clickToPaginateVisualOverlay &&
  ...
  toggleMenu(event: MouseEvent) {
    ...
    if (
      targetElement.getAttribute('onclick') !== null ||
      targetElement.getAttribute('href') !== null ||
      targetElement.getAttribute('role') !== null ||
      targetElement.getAttribute('kavita-part') != null
    ) {
