...
/// Kavita.Common.Extensions.ImageExtensions. Helpers that the parity tests need to reach are
/// marked <c>internal</c>; flip them back to <c>private</c> on the lift (nothing outside this
...
public static class ImageExtensions
{
    ...
    /// </summary>
    internal sealed class ImageQualityMetrics
    {
    ...
    {
        // Match the oracle: if the dimensions differ, resample img2 up/down to img1's size,
        // then compare pixel-by-pixel over the RGB bands (alpha is ignored).
        ...
        // Avg() is the mean over every band-element. The oracle sums R²+G²+B² per pixel and
        // divides by the pixel count only, so multiply the per-element mean back up by the band count.
    ...
    /// </summary>
    internal static ImageQualityMetrics GetImageQualityMetrics(Image image)
    {
        // Create a smaller version if the image is large to speed up analysis.
        // ResizeMode.Max in the oracle bounds the longest side to 512 while preserving aspect ratio.
    ...
    /// </summary>
    internal static (bool IsColor, double Colorfulness) AnalyzeColorfulness(RgbBuffer image)
    {
    ...
    /// </summary>
    internal static double CalculateContrast(RgbBuffer image)
    {
        // For performance, sample a subset of pixels
        var sampleSize = Math.Min(1000, image.Width * image.Height);
        var stepSize = Math.Max(1, (image.Width * image.Height) / sampleSize);
    ...
    /// </summary>
    internal static double EstimateSharpness(RgbBuffer image)
    {
    ...
    /// </summary>
    internal static double EstimateNoiseLevel(RgbBuffer image)
    {
    ...
    /// <summary>
    /// Reads width/height from an image header without decoding the pixel data (mirrors the
    /// oracle's <c>Image.Identify</c>). NetVips loads lazily, so reading the properties is cheap.
    /// </summary>
    ...
    /// fixed R,G,B layout. ImageSharp's <c>Image.Load&lt;Rgba32&gt;</c> did this implicitly; NetVips
    /// loads files as-is (1-band grayscale, 4-band CMYK, etc.), so we normalize explicitly.
    /// </summary>
    ...
    /// <summary>
    /// Test seam: load a file straight into an <see cref="RgbBuffer"/> for per-metric parity checks.
    /// </summary>
    internal static RgbBuffer LoadRgbBuffer(string path)
    {
        using var img = Image.NewFromFile(path);
        return ToRgbBuffer(img);
    }
    ...
    /// the one native round-trip; all pixel loops index the managed buffer, never Getpoint().
    /// </summary>
...
/// <summary>
/// A fixed-layout, row-major, 3-band (R,G,B) 8-bit view over a materialized NetVips image. Replaces
/// ImageSharp's <c>Image&lt;Rgba32&gt;</c> indexer for the ported metric loops. Indexing is managed
/// array access, so the pixel-heavy oracle algorithms port over line-for-line without native calls.
/// </summary>
internal readonly struct RgbBuffer
{
    public int Width { get; }
    public int Height { get; }
    private readonly byte[] _data;

    public RgbBuffer(int width, int height, byte[] data)
    {
        Width = width;
        Height = height;
        _data = data;
    ...
    public Pixel this[int x, int y]
    {
        get
        {
            ...
            return new Pixel(_data[offset], _data[offset + 1], _data[offset + 2]);
        }
    }
}

/// <summary>A single RGB sample, mirroring the <c>.R/.G/.B</c> access the oracle loops use.</summary>
